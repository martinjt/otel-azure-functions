...
echo "  --ports 4318 \ "
echo "  --azure-file-volume-account-name $STORAGE_ACCOUNT_NAME \ "
echo "  --azure-file-volume-account-key $STORAGE_ACCOUNT_KEY \ "
echo "  --azure-file-volume-share-name $STORAGE_SHARE_NAME \ "
echo "  --azure-file-volume-mount-path /etc/otelcol \ "
echo "  --secure-environment-secrets \"honeycomb-api-key=$HONEYCOMB_API_KEY\" \ "
echo "  --environment-variables \"HONEYCOMB_API_KEY=secretref:honeycomb-api-key\" \"HONEYCOMB_LOGS_DATASET=azure-logs\" \ "
...
az container create \
  ...
  --azure-file-volume-mount-path /etc/otelcol \
  --secure-environment-variable "HONEYCOMB_API_KEY=$HONEYCOMB_API_KEY" \
  --environment-variables "HONEYCOMB_API_KEY=secretref:honeycomb-api-key" "HONEYCOMB_LOGS_DATASET=azure-logs" \
  --vnet-name $VNET_NAME \
  --subnet $SUBNET_NAME
